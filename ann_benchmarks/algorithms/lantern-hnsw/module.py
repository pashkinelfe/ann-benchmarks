...
class PGLantern(BaseANN):
    ...
    def fit(self, X):
        ...
        cur = conn.cursor()
        cur.execute("CREATE TABLE items (id int, embedding real[1536])")
...
#               copy.write_row((i, embedding))
        ...
        if self._metric == "angular":
            cur.execute(
                "CREATE INDEX hnsw_idx ON items USING hnsw (embedding dist_cos_ops) WITH (M=%d, ef_construction=64, ef=4, dim=1536)"
                % self._lists
            )
    ...
    def set_query_arguments(self, probes):
        self._probes = probes
        self._cur.execute("ALTER INDEX hnsw_idx SET ( ef = %d )" % probes)
